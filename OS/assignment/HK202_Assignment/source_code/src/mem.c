...
// 			int j;
// 			for (j = 0; j < proc->seg_table->table[j].pages->size; j++)
// 			{
// 				if (proc->seg_table->table[j].v_index == get_second_lv(virtual_adrr))
// 				{
// 					int prev_page = proc->seg_table->table[j].pages->size -= 1;
// 					proc->seg_table->table[j].pages->table[j] = proc->seg_table->table[j].pages->table[prev_page];
// 				}
// 			}

// 			if (proc->seg_table->table[j].pages->size == 0) //free page table - adapted
// 			{
...
// 				{
// 					if (proc->seg_table->table[i].v_index == segIndex)
// 					{
// 						int prev_idx = proc->seg_table->size - 1;
// 						proc->seg_table->table[i] = proc->seg_table->table[prev_idx];
// 						proc->seg_table->table[prev_idx].v_index = 0;
// 						free(proc->seg_table->table[prev_idx].pages);
...
// 		}
// 		//*break pointer ?
// 		if (virtual_adrr + num_pages * PAGE_SIZE == proc->bp)
// 		{
// 			while (proc->bp >= PAGE_SIZE)
// 			{
// 				addr_t last_addr = proc->bp - PAGE_SIZE;
...
//     { // check address is valid and get physical_addr
