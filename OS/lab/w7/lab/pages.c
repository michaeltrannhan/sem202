...
int translate(address_t virtual, address_t *physical)
{
	...
	// to physical address.
	// int v_index = get_index(virtual);
	// int v_offset = get_offset(virtual);
	// int i;
	// for (i = 0; i < TLB_SIZE; i++)
	// {
	// 	if (tlb[i].virtual == v_index)
	// 	{
	// 		*physical = tlb[i].physical << OFF_LEN | v_offset;
	// 		tlb[i].count++;
	// 		return 1;
	// 	}
	// }
	...
	// {
	// 	if (page_table[i].virtual == v_index)
	// 	{
	// 		struct tlb_slot_t *bad = get_least_accessed_slot();
	// 		if (bad)
	// 		{
	// 			bad->virtual = page_table[i].virtual;
	// 			bad->physical = page_table[i].physical;
	// 			bad->count = page_table[i].count + 1;
	// 			*physical = page_table[i].physical << OFF_LEN | v_offset;
	// 			return 1;
	// 		}
	// 	}
	// }

	index_t index = get_index(virtual);
	offset_t offset = get_offset(virtual);
	int i = 0;

	// TLB hit
	for (i = 0; i < TLB_SIZE; i += 1)
	{
		if (index == tlb[i].virtual)
		{
			*physical = (tlb[i].physical << OFF_LEN) + offset;
			tlb[i].count += 1;
	...
	// TLB miss
	struct tlb_slot_t *lslot = get_least_accessed_slot();
	int ret_flag = 0;
	for (i = 0; i < PAGE_TABLE_SIZE; i += 1)
	{
		// Write back because tlb is not pointer to pointer
		if (lslot->virtual == page_table[i].virtual)
		{
			page_table[i] = *lslot;
			ret_flag += 1;
		}

		if (index == page_table[i].virtual)
		{
			*physical = (page_table[i].physical << OFF_LEN) + offset;
